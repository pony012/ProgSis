...
class Ensamblador:
	...
	def cargarArchivo(self, rutaArchivo):
		...
		if(Util.existeArchivo(rutaArchivo)):
			...
			fileListado = Listado(nombreListado)
			...
			for line in fileText.split('\n'):
				...
				if (currentLine.comentario) :
					...
					if str(currentLine.codop).upper() == "EQU":
						if(currentLine.etiqueta != None and currentLine.operando != None):
							...
							if("Directo" in analisisCodop.modos or "Extendido" in analisisCodop.modos):
								...
								if(not tabsim.write(currentLine.etiqueta, analisisCodop.valor)):
									errorTextBox += str(lineCount - 1) + ": La etiqueta "+str(currentLine.etiqueta)+" ya fue usada\n"
								#Se escribe en el archivo de listado
								fileListado.write(analisisCodop.valor,currentLine.etiqueta,"EQU",currentLine.operando)
						else:
					...
					if str(currentLine.codop).upper() == "ORG":
						if(currentLine.operando == None):
							...
							if("Directo" in analisisCodop.modos or "Extendido" in analisisCodop.modos):
								...
								fileListado.write(contLoc,currentLine.etiqueta,"ORG",currentLine.operando)
							else:
								errorTextBox += str(lineCount - 1) + ": El operando de la directiva ORG tiene que ser un número en cualquier representación válida (binario, octal, decimal o hexadecimal)\n"
					...
					if str(currentLine.codop).upper() == "END":
						...
						banderaEnd = True
						fileListado.write(contLoc,currentLine.etiqueta,currentLine.codop,currentLine.operando)
						if not banderaOrg:
					...
					if(currentLine.codop != None):
						...
						if not currentLine.codop.text.upper() in self.tabop:
							...
							if not permiteOperando and currentLine.operando == None:
								fileListado.write(contLoc,currentLine.etiqueta,currentLine.codop,currentLine.operando)
								contLoc += self.tabop[currentLine.codop.text.upper()]['modos']['Inherente']['bTotal']
							...
							if len(analisisCodop.errores) != 0:
								...
								for modo in analisisCodop.modos:
									if modo in self.tabop[currentLine.codop.text.upper()]['modos'] and not banderaModoEncontrado:
										...
										if modo == 'IDX':
											fileListado.write(contLoc,currentLine.etiqueta,currentLine.codop,currentLine.operando)
											contLoc += self.tabop[currentLine.codop.text.upper()]['modos']['IDX']['bTotal']
											...
											fileListado.write(contLoc,currentLine.etiqueta,currentLine.codop,currentLine.operando)
											contLoc += self.tabop[currentLine.codop.text.upper()]['modos']['IDX1']['bTotal']
											variablesTextBox += "Indizado de 9 bits, (IDX1), " + str(self.tabop[currentLine.codop.text.upper()]['modos']['IDX1']['bTotal']) + " bytes\n"
										elif modo == 'IDX2':
											fileListado.write(contLoc,currentLine.etiqueta,currentLine.codop,currentLine.operando)
											contLoc += self.tabop[currentLine.codop.text.upper()]['modos']['IDX2']['bTotal']
											...
											fileListado.write(contLoc,currentLine.etiqueta,currentLine.codop,currentLine.operando)
											contLoc += self.tabop[currentLine.codop.text.upper()]['modos']['[D,IDX]']['bTotal']
											...
											fileListado.write(contLoc,currentLine.etiqueta,currentLine.codop,currentLine.operando)
											contLoc += self.tabop[currentLine.codop.text.upper()]['modos']['REL']['bTotal']
											...
											fileListado.write(contLoc,currentLine.etiqueta,currentLine.codop,currentLine.operando)
											contLoc += self.tabop[currentLine.codop.text.upper()]['modos']['Extendido']['bTotal']
											variablesTextBox+="Extendido, de "+str(self.tabop[currentLine.codop.text.upper()]['modos']['Extendido']['bTotal']) + " bytes\n"
										elif modo == 'Directo':
											fileListado.write(contLoc,currentLine.etiqueta,currentLine.codop,currentLine.operando)
											contLoc += self.tabop[currentLine.codop.text.upper()]['modos']['Directo']['bTotal']
											variablesTextBox+="Directo, de "+str(self.tabop[currentLine.codop.text.upper()]['modos']['Directo']['bTotal']) + " bytes\n"
										elif modo == 'Inmediato':
											fileListado.write(contLoc,currentLine.etiqueta,currentLine.codop,currentLine.operando)
											contLoc += self.tabop[currentLine.codop.text.upper()]['modos']['Inmediato']['bTotal']
